...
class ContextProcessor:
    ...
    def process(self, query: str, documents: List[Dict[str, Any]], query_type: str = "general") -> str:
        ...
        """
        logger.info(f"Processing context for {len(documents)} documents, query_type: {query_type}")
        
        # إذا كان نوع الاستعلام هو تحية، نعيد سياقًا خاصًا للتحية
        if query_type == "تحية":
            return "هذا استعلام تحية. يرجى الرد بتحية مناسبة وتوضيح قدراتك في حل المسائل الرياضية."
